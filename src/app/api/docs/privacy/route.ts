...
export async function GET() {
  ...
  const url = process.env.PRIVACY_PDF_URL || 'https://storage.googleapis.com/solas-site-docs/privacy-policy.pdf';
  ...
  try {
    ...
    if (!res.ok || !res.body) {
      return NextResponse.json({ error: 'Unable to fetch privacy policy' }, { status: 502 });
    }
    ...
    headers.set('Content-Disposition', 'inline; filename="privacy-policy.pdf"');
